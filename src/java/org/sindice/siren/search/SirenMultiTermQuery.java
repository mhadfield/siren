...
public abstract class SirenMultiTermQuery extends SirenPrimitiveQuery {
	...
	private static final long serialVersionUID = 2676411443159499211L;

	@Override
	public Weight createWeight(Searcher searcher) throws IOException {
		return new Weight() {
			
			private static final long serialVersionUID = -2712451355313240121L;
			...
			public Query getQuery() {
				return SirenMultiTermQuery.this;
			}
	...
	private static final class ConstantScoreFilterRewrite extends RewriteMethod implements Serializable {
	    ...
	    public Query rewrite(IndexReader reader, SirenMultiTermQuery query) {
	      Query result = new SirenConstantScoreQuery(new SirenMultiTermQueryWrapperFilter<SirenMultiTermQuery>(query), (SirenNumericRangeQuery<?>) query);
	  ...
	  private static class ConstantScoreBooleanQueryRewrite extends ScoringBooleanQueryRewrite implements Serializable {
	    ...
	    public Query rewrite(IndexReader reader, SirenMultiTermQuery query) throws IOException {
	      ...
	      Query result = new SirenConstantScoreQuery(new QueryWrapperFilter(super.rewrite(reader, query)), (SirenNumericRangeQuery<?>) query);
	      result.setBoost(query.getBoost());
	      return result;
	    }
...
//	      FilteredTermEnum enumerator1 = query.getEnum(reader);
//
//	      do {
//	    	  Term term = enumerator1.term();
//	    	  System.out.println("Term: " + term);
//	      } while(enumerator1.next());
	      ...
	      try {
	        while(true) {
	          ...
	          if (pendingTerms.size() >= termCountLimit || docVisitCount >= docCountCutoff) {
	            ...
	            // Strip scores
	            Query result = new SirenConstantScoreQuery(new QueryWrapperFilter(bq), (SirenNumericRangeQuery<?>) query);
	            result.setBoost(query.getBoost());
