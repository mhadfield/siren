...
extends SirenScorer {
  ...
  throws IOException {
    ...
    while (queueSize > 0) {
      if (scorerCellQueue.topEntity() > entityID ||
         (scorerCellQueue.topEntity() == entityID && scorerCellQueue.topTuple() >= tupleID) ||
         (scorerCellQueue.topEntity() == entityID && scorerCellQueue.topTuple() == tupleID && scorerCellQueue.topCell() >= cellID)) {
        entity = scorerCellQueue.topEntity();
        tuple = scorerCellQueue.topTuple();
        cell = scorerCellQueue.topCell();
        return entity;
...
//  throws IOException {
//    final Explanation res = new Explanation();
//    final Iterator<SirenPrimitiveScorer> ssi = scorers.iterator();
//    float sumScore = 0.0f;
//    int nrMatches = 0;
//    while (ssi.hasNext()) {
//      final Explanation es = (ssi.next()).explain(doc);
...
//    res.setDescription("sum over " + nrMatches + ":");
//    return res;
//  }
