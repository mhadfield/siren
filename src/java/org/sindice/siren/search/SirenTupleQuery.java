...
extends Query {
  ...
  private class SirenTupleWeight extends Weight {
    ...
    throws IOException {
      ...
      for (int i = 0 ; i < weights.size(); i++) {
        final SirenTupleClause c = clauses.get(i);
        final Weight w = weights.get(i);
        final SirenBooleanScorer subScorer = (SirenBooleanScorer) w.scorer(reader, true, false);
        if (subScorer != null) {
  ...
  throws IOException {
    ...
    // into a single {@link SirenTermQuery} without any tuple index constraint.
	  //DK
	  for( SirenTupleClause clause : clauses ) {
		  clause.setQuery( (SirenCellQuery) clause.getQuery().rewrite(reader) );
	  }
  ...
  public String toString(final String field) {
    ...
    for (int i = 0; i < clauses.size(); i++) {
      ...
      if (subQuery != null) {
        if (subQuery instanceof SirenTupleQuery) { // wrap sub-bools in parens
          buffer.append("(");
