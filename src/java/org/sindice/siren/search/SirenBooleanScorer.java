...
extends SirenScorer {
  ...
  extends SirenPrimitiveScorer {
    ...
    private int doNext() throws IOException {
      boolean more = true;
      int cellID = scorer.cell();

      while (more && (cellID < cellConstraintStart ||
                      cellID > cellConstraintEnd)) {
    ...
    public int nextPosition() throws IOException {
      ...
      } while (more && (scorer.cell() < cellConstraintStart ||
                        scorer.cell() > cellConstraintEnd));
      if (more) {
        return 0; // position is invalid in this scorer, return 0.
      }
      else {
