...
extends SirenScorer {
  ...
  throws IOException {
    ...
    if (firstTimeOptScorer) {
      ...
      if (optScorer.advance(curEntity) == NO_MORE_DOCS) {
        optScorer = null;
        return reqScore;
      }
    }
    ...
    else if ((optScorer.entity() < curEntity) && (optScorer.advance(curEntity) == NO_MORE_DOCS)) {
      optScorer = null;
      return reqScore;
    }

    return (optScorer.entity() == curEntity) ? reqScore + optScorer.score()
                                             : reqScore;
  }
