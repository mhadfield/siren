...
extends Query {
  ...
  public static void setMaxClauseCount(final int maxClauseCount) {
    ...
    SirenCellQuery.maxClauseCount = maxClauseCount;
  }

  private ArrayList<SirenCellClause> clauses = new ArrayList<SirenCellClause>();
   ...
   */
  public SirenCellQuery(final boolean disableCoord) {
   ...
   */
  public void add(final SirenPrimitiveQuery query, final SirenCellClause.Occur occur) {
    this.add(new SirenCellClause(query, occur));
  }
   ...
   * @see #getMaxClauseCount()
   */
  public void add(final SirenCellClause clause) {
    if (clauses.size() >= maxClauseCount) throw new TooManyClauses();
    clauses.add(clause);
  }

  /** Returns the set of clauses in this query. */
  public SirenCellClause[] getClauses() {
    return clauses.toArray(new SirenCellClause[clauses.size()]);
  }

  /** Returns the list of clauses in this query. */
  public List<SirenCellClause> clauses() {
    return clauses;
  }
  ...
  private class SirenCellWeight extends Weight {
    ...
    public SirenCellWeight(final Searcher searcher) throws IOException {
      ...
      for (int i = 0; i < clauses.size(); i++) {
        final SirenCellClause c = clauses.get(i);
    ...
    throws IOException {
      ...
      sum *= SirenCellQuery.this.getBoost() * SirenCellQuery.this.getBoost(); // boost each sub-weight
    ...
    throws IOException {
      ...
      for (int i = 0 ; i < weights.size(); i++) {
        ...
        if(scorer instanceof SirenPrimitiveScorer) {
        	final SirenPrimitiveScorer subScorer = (SirenPrimitiveScorer) scorer;
        	if (subScorer != null) {
...
//        	}
  ...
  throws IOException {
    ...
    // into a single {@link SirenTermQuery} without any cell index constraint.
	  ...
	  for(SirenCellClause cellClause : clauses) {
		  SirenPrimitiveQuery query = cellClause.getQuery();
		  if( query instanceof SirenNumericRangeQuery) {
			  cellClause.setQuery((SirenPrimitiveQuery) query.rewrite(reader));
		  }
	  }
  ...
  public void extractTerms(final Set<Term> terms) {
    for (final SirenCellClause clause : clauses) {
  ...
  public Object clone() {
    ...
    clone.clauses = (ArrayList<SirenCellClause>) this.clauses.clone();
    return clone;
  }
  ...
  public String toString(final String field) {
    ...
    for (int i = 0; i < clauses.size(); i++) {
      ...
      if (subQuery != null) {
        if (subQuery instanceof SirenCellQuery) { // wrap sub-bools in parens
          buffer.append("(");
